...
const sumTotal = (transactions) => transactions.reduce((partialSum, transaction) => partialSum + transaction.sum, 0)

const MonthsOverviewScreen = () => {
    ...
    useEffect(() => {
        if (isFocused) {
            getCategoriesTotalByDate(startOfYear(new Date()), endOfYear(new Date())).then(r => {
